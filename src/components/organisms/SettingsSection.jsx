...
export default function SettingsSection({ section = 'pagetypes' }) {
  return (
    <div>
                          ...
                          <div key={rIdx}>
                            ...
                            <div className={styles.condRow}>
                              ...
                              <select className={styles.condOpSel} value={rule.op} onChange={e => updateRule(pt.id, rIdx, 'op', e.target.value)}>
                                <option>contains</option><option>starts with</option><option>ends with</option>
                                <option>matches regex</option><option>equals</option>
                              </select>
                              <input className={styles.condInput} value={rule.value} onChange={e => updateRule(pt.id, rIdx, 'value', e.target.value)} />
                              <button className={styles.condDeleteBtn} onClick={() => deleteRule(pt.id, rIdx)} title="Remove">
...
function AddRuleModal({ pageType, onClose }) {
  ...
  function addCondition() { setConditions(prev => [...prev, { op: 'contains', value: '' }]) }
  function deleteCondition(i) { setConditions(prev => prev.filter((_, idx) => idx !== i)) }
  function updateCondition(i, field, val) { setConditions(prev => prev.map((c, idx) => idx === i ? { ...c, [field]: val } : c)) }
