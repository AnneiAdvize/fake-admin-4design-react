...
const clients = ['Brand - EN', 'Brand - FR']
    ...
    <nav className={styles.nav}>
      <div className={styles.inner}>
        ...
        <div className={styles.items}>
          ...
          <NavLink to="/engagement" className={({ isActive }) => `${styles.item} ${isActive ? styles.active : ''}`}>
            <IconEngagement />
            Engagement
          </NavLink>
          <NavLink to="/shopping-assistant" className={({ isActive }) => `${styles.item} ${isActive ? styles.active : ''}`}>
            <IconShopping />
            Shopping Assistant
          </NavLink>
          <NavLink to="/knowledge" className={({ isActive }) => `${styles.item} ${isActive ? styles.active : ''}`}>
            <IconKnowledge />
            Knowledge
          </NavLink>
        ...
        <div className={styles.actions}>
          ...
          <button className={styles.btnNotification} aria-label="Notifications">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              <path fillRule="evenodd" clipRule="evenodd" d="M18 9V12L20.2663 14.7196C21.3519 16.0223 20.4256 18 18.7299 18H5.27006C3.57438 18 2.64807 16.0223 3.73361 14.7196L5.99997 12V9C5.99997 5.68629 8.68627 3 12 3C15.3137 3 18 5.68629 18 9ZM5.27006 16L7.99997 12.7241V9C7.99997 6.79086 9.79084 5 12 5C14.2091 5 16 6.79086 16 9V12.7241L18.7299 16L5.27006 16Z" fill="#3D3D38"/>
              <path d="M15 19H9V21H15V19Z" fill="#3D3D38"/>
            </svg>
          </button>

          <div className={styles.clientSelector} ref={dropdownRef}>
            <button
              className={`${styles.clientBtn} ${dropdownOpen ? styles.clientBtnOpen : ''}`}
              onClick={() => setDropdownOpen(o => !o)}
              aria-haspopup="true"
              aria-expanded={dropdownOpen}
            >
              <span className={styles.clientName}>{selectedClient}</span>
              <svg className={`${styles.chevron} ${dropdownOpen ? styles.chevronOpen : ''}`} width="10" height="6" viewBox="0 0 10 6" fill="none">
            ...
            {dropdownOpen && (
              <div className={styles.dropdown}>
                {clients.map(c => (
                  <div
                    key={c}
                    className={`${styles.dropdownItem} ${c === selectedClient ? styles.dropdownItemActive : ''}`}
          ...
          <div className={styles.avatar} title="Anne Pedro">AP</div>
        </div>
      </div>
    </nav>
  )
...
function IconHome() {
  return (
    <svg width="14" height="15" viewBox="0 0 14 15" fill="none" className={styles.icon}>
